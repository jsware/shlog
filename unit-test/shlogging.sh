...
grep -vE "$(date --utc +"^%a %b %d [0-9]{2}:[0-9]{2}:[0-9]{2} %Z %Y	`hostname`	`whoami`	shlogging.loop	[0-9]{1,5}	INFO	Loop count [0-9]$")" $SHLOG_FILE && die "Found UNEXPECTED output in $SHLOG_FILE"
